...
%% Parameters
descriptor = 'cnn_whole_layer2';
descriptorSize = 96*56*56;
distance = 'L1';
k = 1;
...
numSplits = 5;
%%
tally = zeros(5,numTest*numel(styleNames));
...
for split = 1:numSplits
    ...
    fprintf('done.\nTesting...');
    %% Testing
    testData = zeros(size(allDescriptors,1), size(testFileList,1)*size(testFileList,2),'single');
    testFileNums = zeros(1,size(testFileList,1)*size(testFileList,2));
    count = 0;
    for style = 1:size(testFileList,1)
...
save([savePath '/confusion.mat'], 'conf');
...
for style = 1:numel(styleNames)
    ...
    total = 0;
    for split = 1:numSplits
        total = total + sum(tally(split, range))/numTest;
    end
